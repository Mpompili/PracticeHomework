...
class MyHashSet
  ...
  def initialize(*array)
    @store = {}
    array.flatten.each { |el| self.insert(el) } if array.is_a?(Array)
  end
  ...
  def union(set)
    var = @store.merge(set.store).keys.sort
    self.class.new(var)
  end

  def intersect(set)
    answer = @store.keys & set.store.keys
    self.class.new(answer)
  end

  def minus(set)
    proto = dup
    set.store.each_key { |key| proto.delete(key) }
    self.class.new(proto.store.keys)
  end
end
