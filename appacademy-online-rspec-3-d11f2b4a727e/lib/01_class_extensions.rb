...
class Hash
  def difference(other_hash)
    delete_these = self.keys & other_hash.keys
    answer_hash = self.merge(other_hash)
    delete_these.each do |el|
      answer_hash.delete(el)
    end
    answer_hash
  end
end
...
class Fixnum
  def stringify(base)
    hexhash = Hash.new
    nums = (0..15).to_a
    values = (0..9).to_a.map(&:to_s) + ('a'..'a').to_a
    16.times { |idx| hexhash [nums[idx]] = values[idx] }
    ...
    when 16
      ...
      start_point = dup
      until start_point <= 0
        answer << (start_point % 16)
        start_point /= 16
      end
      answer.map! { |cypher| hexhash[cypher] }.reverse!
    end
    answer.join
  end
end
